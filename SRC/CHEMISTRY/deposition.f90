module deposition
   ...
   use grid, only: z, nx, ny, nzm, dimx1_s, dimx2_s, dimy1_s, dimy2_s
...
CONTAINS
   ...
   subroutine dry_deposition_driver(gchem_field, canopy_index, g_depos_horiz_mean_tend_ISOPOOH, g_depos_horiz_mean_tend_IEPOX)
      
      real, intent(inout) :: gchem_field(dimx1_s:dimx2_s, dimy1_s:dimy2_s, nzm, NVAR)     ! Must specify size due to interface issue!
      integer, intent(in) :: canopy_index                                       ! Index of box corresponding to canopy
      ...
      bottommost_layer_height = (z(canopy_index + 1) - z(canopy_index))*100                         ! Calculates height of lowest box; converts from m to cm b/c velocities in cm/s
      ...
      do v_selected = 1, NVAR                                                 ! v_selected is the index of the deposition array
         do v = 1, NVAR                                                      ! v is the index of the species list (e.g. gchem_fields)
            if (dry_deposition_species(v_selected) == trim(SPC_NAMES(v))) then
               ...
               do i = 1, nx
                  do j = 1, ny
                     gchem_field(i, j, canopy_index, v) = &
                        (1.-(dry_deposition_velocities(v_selected)*dtn)/bottommost_layer_height)*gchem_field(i, j, canopy_index, v)
                     ! JY: units are (1 - ( cm/s * s ) / (cm))
                     ! This assumes that the lowest box is completely well-mixed
                     gchem_field(i, j, canopy_index, v) = max(gchem_field(i, j, canopy_index, v), 0.0)         ! Prevents negative mixing ratios
                  end do
               end do
