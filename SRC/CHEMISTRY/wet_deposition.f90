module wet_deposition
   ...
   real, parameter  :: areal_fraction = 1
   real, parameter :: R = 0.08205                         ! atm M-1 K-1
   real, parameter :: fraction_in_ice_phase = 0           ! no ice
   real, parameter :: epsilon = 1                         ! Retention factor, 1 because we are ignoring ice!
...
CONTAINS
   ...
   subroutine convective_scavenging(gchem_field, M_profile, change_in_qcl, change_in_qpl)
      ...
      cloud_mixing_ratio_threshold = 0.01                  ! If we are in a cloud, in [g/kg]
