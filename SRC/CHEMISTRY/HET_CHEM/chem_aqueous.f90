...
contains
   ...
   subroutine iepox_aqueous_tendencies(nzm, T, p, Rp, Wl, H, NO3, SO4, HSO4, &
      ...
      real, intent(in) :: T(1:nzm)   !  K
      real, intent(in) :: p(1:nzm)   ! atm
      real, intent(in) :: Rp(1:nzm)  ! droplet radius (m)
      real, intent(in) :: Wl(1:nzm)  ! liquid water content vol H2O/vol air
      real, intent(in) :: H(1:nzm)   !  M of H+
      real, intent(in) :: NO3(1:nzm)   !  M
      real, intent(in) :: SO4(1:nzm)   !  M
      real, intent(in) :: HSO4(1:nzm)   !  M

      real, dimension(1:nzm, 1:naqchem_fields), intent(in) :: gas_input, aq_input         ! gas_input in ppv * atm; aq_input in M / qc
      real, dimension(1:nzm, 1:naqchem_fields), intent(out) :: gas_tend, aq_tend       

      logical, intent(inout) :: print_debug_output
      logical :: add_text
      ...
      if (print_debug_output) then
         add_text = .true.
         do k = 1, nzm
            if (abs(gas_tend(k,iIEPOX)) > 1.e3) then
